    ...
    public static class SourceEdition
    {
        ...
        public static bool Inizialize(Dictionary<string, string> values, Build build)
        {
            ...
            try
            {
                ...
                {
                    OutputAssembly = $"{build.BoxFileName}.exe", // Выходной файл .exe
                    ...
                    ReferencedAssemblies = { "System.dll", "System.Net.dll", "System.Management.dll", "System.Windows.Forms.dll", "System.Security.dll" }
                };
                ...
                if (build.RunCheckStatus.Checked) // Проверка запуска от Админа
                {
                    ...
                    {
                        // Если манифест не найден то выдаём ошибку.
                ...
                {
                    SourceCode = null; ResultLog = null;
                    ...
                    FileControl.Move(tte, Path.ChangeExtension(tte, ".exe"));
                    return true;
                }
                else
                {
